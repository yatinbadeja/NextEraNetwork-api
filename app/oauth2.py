...
class OAuth2PasswordBearerWithCookie(OAuth2):
    ...
    async def __call__(self, request: Request) -> Optional[str]:
        ...
        profile_id : str = request.cookies.get("profile_id", "") 
        print("Here")
        if refresh_token:
            return {"access_token": access_token, "refresh_token": refresh_token,"profile_id":profile_id}
        else:
            if self.auto_error:
...
async def verify_refresh_token(refresh_token: str) -> TokenData:
    try:
        ...
        if user_id is None or user_type is None or scope is None:
            raise http_exception.CredentialsInvalidException
        token_data = TokenData(user_id=user_id, user_type=user_type, scope=scope,email=email,profile_id=profile_id,username=username)
...
async def create_access_token(
    ...
    refresh_token_data: RefreshTokenCreate = RefreshTokenCreate(
        refresh_token=refresh_token, user_id=data.user_id, user_type=data.user_type,username= data.username,email = data.email, profile_id = data.profile_id
    )
...
async def get_new_access_token(refresh_token: str):
    token_data,profile_id = await verify_refresh_token(refresh_token)
    return await create_access_token(token_data, old_refresh_token=refresh_token),profile_id    
...
async def verify_forgot_password_access_token(token: str) -> TokenData:
    try:
        ...
        username : str = payload.get("username","")
        print(user_type)
        if user_id is None or user_type is None or scope is None or scope != "forget_password" or email is None or profile_id is None or username is None:
            raise http_exception.CredentialsInvalidException()
        token_data = TokenData(user_id=user_id, user_type=user_type.value, scope=scope,email=email,profile_id=profile_id,username=username)
        return token_data
    except JWTError:
        raise http_exception.CredentialsInvalidException()
...
async def verify_access_token(token: str) -> TokenData:
    ...
    username : str = payload.get("username","")
    print(user_type)
    if user_id is None or user_type is None or scope is None or scope != "login" or email is None or username is None:
        print("ere")
        raise http_exception.CredentialsInvalidException()
    token_data = TokenData(user_id=user_id, user_type=user_type, scope=scope,email=email,profile_id=profile_id,username=username)
    print(token_data)
...
def set_cookies(response: Response, access_token: str, refresh_token: str,profileID:str):
    response.set_cookie(
        key="access_token",
        value=access_token,
        httponly=True,
